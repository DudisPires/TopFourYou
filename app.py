...
try:
    # Adicionamos scrape_watchlist para a nova funcionalidade
...
# --- NOVA ROTA PARA SERVIR A PÁGINA 'common.html' ---
@app.route('/common')
def common_page():
...
def recommend():
    ...
    try:
        # 1. Busca o avatar e os filmes favoritos em paralelo (ou sequencialmente)
        ...
        # ... (lógica de match e recomendação continua a mesma)
        ...
        # 2. Prepara o objeto de resposta com avatar e recomendações
        response_data = {
            "avatar_url": avatar_url,
            "recommendations": recommendations_df.to_dict(orient="records")
        }
        
        return jsonify(response_data) # Retorna o novo objeto combinado

    except Exception as e:
        print(f"Erro ao gerar recomendações: {e}")
        return jsonify({"error": "Erro interno durante a recomendação."}), 500

# --- NOVA ROTA PARA A LÓGICA DE ENCONTRAR FILMES EM COMUM ---
...
def find_common():
    ...
    try:
        # Usa a função do scraper para obter a watchlist de cada usuário
        ...
        # Encontra a interseção (filmes em comum) usando conjuntos
